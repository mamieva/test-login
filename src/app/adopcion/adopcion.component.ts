...
export class adopcionComponent implements AfterViewInit {
  model: any = { procedureTypeCode: "SOLICITUD_ADOPCION", procedureFromRequest: {}, procedureHasAttributeValues: [] };
  certificateId: any;
  indice: any = 0;
  isLinear = false;
  ...
  updateProcedure(validate: any) {
    ...
    if (!this.model.id) {
      ...
      this.procedureService.postProcedure(this.model).subscribe((response: any) => {
        ...
        really.router.navigateByUrl('adoption/' + response.id);
  ...
  uploadFile(compressBase64: any, base64: any, attachmentType: any, documentId?: any, addPage?: any) {
    let really = this;
    let file = {
...
@Component({
  ...
  templateUrl: 'adopcionModal.component.html'
})
