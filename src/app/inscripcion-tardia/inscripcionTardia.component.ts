...
@Component({
  selector: 'inscripcionTardia',
  templateUrl: './inscripcionTardia.component.html',
  styleUrls: ['./inscripcionTardia.component.scss']
})
export class inscripcionTardiaComponent implements AfterViewInit {
  model: any = { procedureTypeCode: "SOLICITUD_FUERA_DE_TERMINO", procedureFromRequest: {} };
  certificateId: any;
  notJudicial: boolean = true;
  indice: any = 0;
  isLinear = false;
  ...
  testigo_1: any = {
    actorType: {
      id: 4,
      code: "TESTIGO_1",
      relationship: { code: 'TESTIGO_SOLICITUD_TARDIA' },
    },
    person: {
      docType: {},
      address: {},
  ...
  ngOnInit() {
    ...
    if (this.certificateId) {
      ...
      this.procedureService.getProcedure(this.certificateId).subscribe((response: any) => {
        this.model = response;
        if (this.model.procedureTypeCode == 'SOLICITUD_FUERA_DE_TERMINO_JUDICIAL')
          this.notJudicial = false;
        response.people.forEach((element: any) => {
          if (element.actorType.code == 'TESTIGO_1')
            this.testigo_1 = element;
          if (element.actorType.code == 'TESTIGO_2')
    ...
    // this.lookupService.getAttachmentType('SOLICITUD_FUERA_DE_TERMINO').subscribe((response: any) => {
  ...
  inscripctionJudicial() {
    this.notJudicial = false;
    this.model.procedureTypeCode = 'SOLICITUD_FUERA_DE_TERMINO_JUDICIAL';
    this.updateProcedure(false, true);
    // this.lookupService.getAttachmentType('SOLICITUD_FUERA_DE_TERMINO_JUDICIAL').subscribe((response: any) => {
    //   this.attachmentFiles = response;
  ...
  updateProcedure(validate: any, judicial?: any) {
    ...
    if (!this.model.id) {
      ...
      this.procedureService.postProcedure(this.model).subscribe((response: any) => {
        ...
        really.router.navigateByUrl('inscripcionT/' + response.id);
