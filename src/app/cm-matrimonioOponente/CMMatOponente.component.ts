...
@Component({
  selector: 'CMMatOponente',
  templateUrl: './CMMatOponente.component.html',
  styleUrls: ['./CMMatOponente.component.scss']
})
export class CMMatOponenteComponent implements AfterViewInit {
  model: any = { oppositionHasAttributeValues: [] };
  id: any;
  procedureId: any;
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      this.id = +params.opposition;
    });
    this.model.procedureId = this.procedureId;
    ...
    if (this.id) {
      ...
      this.procedureService.getProcedureOpposition(this.procedureId, this.id).subscribe((response: any) => {
        this.model = response;
        // Traer Atributos del Tramite        
        this.procedureService.getAttributesOpposition(this.procedureId, response.id).subscribe((attributes: any) => {
          this.model.oppositionHasAttributeValues = attributes;
        })
    ...
    if (!this.model.id) {
      this.procedureService.getAttributesOpposition(this.procedureId, 0).subscribe((attr: any) => {
        this.model.oppositionHasAttributeValues = attr;
      }, error => {
  ...
  findMotherWife() {
    ...
    this.personService.getPersonByDocumentSex(this.personMotherWife.docNumber, this.personMotherWife.sex).subscribe((response: any) => {
      ...
      really.motherWife.person.birth.address = { city: {} }
    }, error => {
  ...
  findFatherWife() {
    ...
    this.personService.getPersonByDocumentSex(this.personFatherWife.docNumber, this.personFatherWife.sex).subscribe((response: any) => {
      ...
      really.fatherWife.person.birth.address = { city: {} }
    }, error => {
      really.snackBarMessage(error.error.message, 5000);
    });
  }
  findMotherHusband() {
    ...
    this.personService.getPersonByDocumentSex(this.personMotherHusband.docNumber, this.personMotherHusband.sex).subscribe((response: any) => {
      ...
      really.motherHusband.person.birth.address = { city: {} }
    }, error => {
      really.snackBarMessage(error.error.message, 5000);
    })
  }
  findFatherHusband() {
    ...
    this.personService.getPersonByDocumentSex(this.personFatherHusband.docNumber, this.personFatherHusband.sex).subscribe((response: any) => {
      ...
      really.fatherHusband.person.birth.address = { city: {} }
    }, error => {
      really.snackBarMessage(error.error.message, 5000);
    });
  }
  ...
  updateProcedure(validate: any, noMove?: any) {
    ...
    if (!this.model.id) {
      ...
      this.procedureService.postProcedureOpposition(this.procedureId, this.model).subscribe((response: any) => {
        really.model = response;
        // debugger
        localStorage.setItem('nuevoTramite', 'SI');
        // window.location.href += '/' + response.id; 
        really.router.navigateByUrl('oponentes/' + really.model.procedureId + '/' + response.id);
      ...
      this.procedureService.putProcedureOpposition(this.model.procedureId, this.model, this.model.id).subscribe((response: any) => {
        really.model = response;
        really.pdfValidate = {};
        if (!validate && !noMove)
          really.indice += 1;
        ...
        really.procedureService.getProcedureOpposition(really.procedureId, really.id).subscribe((response: any) => {
          really.model = response;
          really.oponentes = [];
          response.people.forEach((element: any) => {
            if (element.actorType.code.indexOf('OPONENTE') >= 0) {
  ...
  uploadFile(base64: any, attachmentType: any, documentId?: any, addPage?: any) {
    ...
    // console.log(file);
    this.procedureService.uploadAttachmentProcedureOpposition(file).subscribe((response: any) => {
      this.matchDocuments();
  ...
  nextStep() {
    this.router.navigateByUrl('/procedure/' + this.model.procedureId)
  }
  backStep() {
    console.log(this.indice);
    this.indice -= 1;
  ...
  printCertificateOpo() {
    let really = this;
    this.procedureService.getCertificateOpoPDF(this.procedureId, this.model.id).subscribe((response: any) => {
      really.pdfValidate = response;
    }, error => {
      really.snackBarMessage(error.error.message, 6000);
    })
  ...
  addOponente(index: any) {
    ...
    if (index == -1 && this.oponente.person.firstName && this.oponente.person.firstName && this.oponente.person.docNumber) {
      ...
      this.personService.putPerson(this.oponente.person.id, this.oponente.person).subscribe((response: any) => {
  ...
  matchDocuments() {
    ...
    this.procedureService.getDocumentTypeByProcedureOpposition(this.procedureId, this.model.id).subscribe((docTypes: any) => {
  ...
  uploadFilesAttachment(file: any) {
    ...
    this.procedureService.uploadAttachmentProcedureFileOpposition(formData).subscribe((response: any) => {
