...
@Component({
  selector: 'supresionApellido',
  templateUrl: './supresionApellido.component.html',
  styleUrls: ['./supresionApellido.component.scss']
})
export class supresionApellidoComponent implements AfterViewInit {
  model: any = { procedureTypeCode: "SOLICITUD_SUPRESION_APELLIDO", procedureFromRequest: {}, procedureHasAttributeValues: [] };
  certificateId: any;
  indice: any = 0;
  isLinear = false;
  ...
  ngOnInit() {
    ...
    this.procedureService.getAttributesProcedureByCode('SOLICITUD_SUPRESION_APELLIDO').subscribe((attr: any) => {
  ...
  updateProcedure(validate: any) {
    ...
    if (!this.model.id) {
      ...
      this.procedureService.postProcedure(this.model).subscribe((response: any) => {
        ...
        really.router.navigateByUrl('supresionAp/' + response.id);
...
@Component({
  ...
  templateUrl: 'supresionApellidoModal.component.html'
})
